...
# Load full dataset
train_data_full = ImageFolder(root="Dataset/archive/Dataset/Train", transform=base_transform)
test_data_full = ImageFolder(root="Dataset/archive/Dataset/Test", transform=base_transform)

# Get balanced indices
train_fake_indices = get_class_indices(train_data_full, "Fake", 100000)
train_real_indices = get_class_indices(train_data_full, "Real", 100000)
# test_fake_indices = get_class_indices(test_data_full, "Fake", 50000)
# test_real_indices = get_class_indices(test_data_full, "Real", 50000)
...
if __name__ == "__main__":
    ...
    # print(config['model']['image-size'])
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(device)
    model = EfficientViT(config=config, channels=1280, selected_efficient_net = 0)
    model.load_state_dict(torch.load("hybrid_model_v9.pth", map_location=device))
    print("✅ Loaded model weights from hybrid_model_v9.pth")
    model.train()
    ...
    for epoch in range(epoch_num):
        ...
        for images, labels in progress_bar:
            ...
            images1 = overlay_lga_on_tensor_batch(images).to(device)
            optimizer.zero_grad()
        ...
        torch.save(model.state_dict(), "hybrid_model_v10.pth")
        print(f"✅ Saved model to hybrid_model_v10.pth")
